...
def verificar_admissoes_leitos0():
    registrar_log("(1)-INICIANDO ROTINA DE VERIFICAÇÃO DE ADMISSÕES EM NOVOS LEITOS.")
    ...
    try:
        ...
        admissoes_aghu = obter_admissoes_aghu(conn_aghu)        

        novas_admissoes = {
            lid: info for lid, info in admissoes_aghu.items()
            if lid not in admissoes_epimed
        }

        if not novas_admissoes
           registrar_log("Nenhuma nova admissão detectada.")
           print("Nenhuma nova admissão detectada.")
           print("Rotina de inclusão de novas admissoes executada com sucesso!")
           return

        registrar_log(f"{len(novas_admissoes)} nova(s) admissao(oes) detectada(s).")
        print(f"Detectadas {len(novas_admissoes)} nova(s) admissao(oes).")

        for prontuario, info in novas_admissoes.items():
            unitcode, bedcode, dthrinternacao, dthrentradaleito = info

            updatetimestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            status = "pendente"  

            exames_epimed = obter_exames_epimed(conn_epimed)  
            exames_aghu = obter_exames_aghu(conn_aghu)        

            novos_exames = {
                lid: info for lid, info in exames_aghu.items()
                if lid not in exames_epimed
            }

            if not novos_exames
                registrar_log("Nenhuma novo exame detectado.")
                print("Nenhuma novo exame detectado.")
                print("Rotina de inclusão de novos exames executada com sucesso!")
                return

            registrar_log(f"{len(novos_exames)} novo(s) exame(s) detectado(s).")
            print(f"Detectados {len(novos_exames)} novo(s) exame(s).")
            ...
            try:
                ...
                with conn_epimed: #commit e rollback automáticos
                    ...
                    mensagem = gerar_mensagem_hl7(
                        unitcode, bedcode, updatetimestamp, prontuario)
                    )

                    resposta = enviar_mensagem_hl7(log_id, mensagem, conn_epimed)

                    if resposta == "AA":  # ACK de sucesso
                        inserir_admissao_epimed(conn_epimed, leito_id, ind_situacao, activebeddate, disablebeddate)
                        inserir_exame_epimed(conn_epimed, leito_id, ind_situacao, activebeddate, disablebeddate)
                        registrar_log(f"Exame do prontuario {prontuario} enviado com sucesso!", nivel="info")
                    else:
                        msg = f"Erro ao enviar exame do prontuario {prontuario}: ACK recebido com código {resposta}"
                        registrar_log(msg, nivel="error")

            except requests.RequestException as e:
                resposta = str(e)
                msg = f"Erro ao enviar exame - prontuário {prontuario}: {resposta}"
                registrar_log(msg, nivel="error")

        print("Rotina de inclusão de leitos novos executada com sucesso!")

    except Exception as e:
        registrar_log(f"❌ Erro na rotina de verificação de novas admissões: {str(e)}", nivel="error")
        print(f"❌ Erro na rotina de verificação de novas admissões: {str(e)}")
...
def verificar_admissoes_leitos():
    ...
    try:
        admissoes_epimed = obter_admissoes_epimed(conn_epimed)   # dict {prontuario: (unitcode, bedcode, dthrinternacao, dthrentradaleito)}
        admissoes_aghu = obter_admissoes_aghu(conn_aghu)         # dict idem

        # novas admissões = existem no AGHU mas não no Epimed
        novas_admissoes = {
            prontuario: info for prontuario, info in admissoes_aghu.items()
            if prontuario not in admissoes_epimed
        }

        if not novas_admissoes:
            registrar_log("Nenhuma nova admissão detectada.")
            print("Nenhuma nova admissão detectada.")
            print("Rotina de inclusão de novas admissões executada com sucesso!")
            return

        registrar_log(f"{len(novas_admissoes)} nova(s) admissão(ões) detectada(s).")
        print(f"Detectadas {len(novas_admissoes)} nova(s) admissão(ões).")
        ...
        for prontuario, info in novas_admissoes.items():
            ...
            try:
                # 1) Buscar exames no AGHU relacionados à internação
                exames_aghu = obter_exames_aghu(conn_aghu, prontuario, dthrentradaleito)

                # 2) Preparar mensagem HL7 (internação + exames)
                log_id = salvar_log_envio(prontuario, conn_epimed)
                mensagem = gerar_mensagem_hl7(
                    unitcode,
                    bedcode,
                ...
                if resposta == "AA":  # sucesso → grava
                    with conn_epimed:
                        # inserir internação
                        int_id = inserir_admissao_epimed(
                            conn_epimed,
                            prontuario,
                            unitcode,
                            bedcode,
                        ...
                        registrar_log(f"Inserida nova internação (ID={int_id}) para prontuário {prontuario}.")

                        # inserir exames
                        for (idexame, dthrexame) in exames_aghu.keys():
                            exame_id = inserir_exame_epimed(conn_epimed, int_id, idexame, dthrexame)
                            registrar_log(f"Exame {idexame} inserido (ID={exame_id}) para internação {int_id}.")

                        registrar_log(f"Internação {int_id} e exames enviados + gravados com sucesso!", nivel="info")
                else:
                    msg = f"Erro ao enviar internação do prontuário {prontuario}: ACK recebido com código {resposta}. Nenhum dado gravado."
                    registrar_log(msg, nivel="error")

            except requests.RequestException as e:
                resposta = str(e)
                msg = f"Erro de rede ao enviar internação/exames - prontuário {prontuario}: {resposta}. Nenhum dado gravado."
                registrar_log(msg, nivel="error")
